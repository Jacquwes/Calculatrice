...
namespace Calculatrice::Operations {
	...
	{
		return pow(Utils::Constants::euler, number);
	}
	...
	double root(double number, double root)
	{
		//tex:
		//$$\sqrt[n]{x}=x^{\frac{1}{n}}$$
		if (number < 0) throw(Utils::Error{ "Erreur racine", "Le programme n'est pas encore capable d'utiliser cette fonction pour des nombres négatifs" });
